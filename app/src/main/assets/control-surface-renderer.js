...
(function(global){
  ...
  function paintValue(root,value,display){const d=root.__msDescriptor,visual=root.__msVisual;if(!d||!visual)return root;const out=root.querySelector?.(".ms-control-value");if(out&&display!==undefined){const text=String(display);if(out.textContent!==text)out.textContent=text}if(d.control===C.KNOB||d.control===C.DIAL){const angle=rotaryAngle(d,visual,value);if(angle!=null){root.dataset.value=String(value);root.style.setProperty("--ms-angle",`${angle}deg`);const pointer=root.querySelector?.(".ms-control-pointer");if(pointer)pointer.style.transform=`translateX(-50%) rotate(${angle}deg)`}}return root}
  function installValueVisual(root,d,visual){root.__msDescriptor=d;root.__msVisual=visual;if(d.control!==C.KNOB&&d.control!==C.DIAL)return;const initial=d.value?.value??d.value?.default;if(initial!=null)paintValue(root,initial);const out=root.querySelector?.(".ms-control-value");if(!out||!global.MutationObserver)return;new MutationObserver(()=>{const n=Number(out.textContent);if(Number.isFinite(n))paintValue(root,n)}).observe(out,{childList:true,characterData:true,subtree:true})}
