"use strict";
(function(){
const MS=window.MultiSynth||{},M=MS.ModuleManifest,H=MS.HierarchyLibrary,E=MS.RackEngine,F=MS.RackFaceplate,P=()=>window.MultiSynthNodePlane;
...
function addEmptyRack(){const api=P();if(!api)return;try{const c=freeCell(),id=`rack-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`;E.addRack(c.row,c.col,{id});H?.registerRack?.(id,`Rack ${(H?.list?.("rack")||[]).length+1}`);api.placeRack(id,insertPoint);close()}catch(e){api.flash?.(e.message||"COULD NOT ADD RACK")}}
...
function render(){items.innerHTML="";mods?.classList.toggle("active",tab==="modules");racks?.classList.toggle("active",tab==="racks");if(tab==="modules"){for(const m of M?.all||[]){const b=document.createElement("button");b.className=`nodeBrowserItem moduleChoice synthMini ${m.themeKey||""}`.trim();b.style.setProperty("--module-color",m.color||"#789");b.innerHTML=`<span class="miniStatus">ADD</span><strong>${m.displayName.toUpperCase()}</strong><span class="miniDesc">${String(m.category||"MODULE").toUpperCase()}</span>`;b.onclick=()=>{const api=P();if(!api)return;try{api.addModuleType(m.id,insertPoint);close()}catch(e){api.flash?.(e.message||"COULD NOT ADD MODULE")}};items.appendChild(b)}}else{items.appendChild(rackChoice({id:"new",modules:[]},"NEW EMPTY RACK",addEmptyRack));const gs=(E?.graph?.().racks||[]).filter(r=>!String(r.id).startsWith("nodehost-"));for(const r of gs)items.appendChild(rackChoice(r,nameForRack(r.id),()=>{const api=P();if(!api)return;api.placeRack(r.id,insertPoint);close()}))}}
