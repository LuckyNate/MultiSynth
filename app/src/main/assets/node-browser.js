"use strict";
(function(){
const MS=window.MultiSynth||{},M=MS.ModuleManifest,R=MS.RackLibrary,E=MS.RackEngine,F=MS.RackFaceplate,S=MS.ModuleSelectorUI,P=()=>window.MultiSynthNodePlane;
...
function nameForRack(id){try{return R?.list()?.find(x=>String(x.id)===String(id))?.name||String(id)}catch(_){return String(id)}}
...
function addEmptyRack(){const api=P();if(!api)return;try{const c=freeCell(),id=`rack-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`;E.addRack(c.row,c.col,{id});R?.registerRack?.(id);api.placeRack(id,insertPoint);close()}catch(e){api.flash?.(e.message||"COULD NOT ADD RACK")}}
function rackChoice(r,name,onPick){const b=document.createElement("button");b.type="button";b.className="nodeBrowserRackChoice";const visual=F?.render?.(r,{name,compact:true,horizontal:true})||document.createTextNode(name);b.appendChild(visual);b.onclick=onPick;return b}
