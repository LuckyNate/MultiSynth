...
(function(global){
    ...
    function bindGrid(grid,{onOpenRack,onEmptyCell,onPreview}={}){
        let pressTimer=null,start=null,dragging=false,rackId=null,scrolling=false,scrollEl=null,startScroll=0;
        const clear=()=>{if(pressTimer)clearTimeout(pressTimer);pressTimer=null;start=null;dragging=false;rackId=null;scrolling=false;scrollEl=null;startScroll=0;};
        grid.addEventListener("pointerdown",e=>{if(zoomGesture())return;const cell=e.target.closest("[data-rack-row][data-rack-col]");if(!cell)return;const rack=cell.querySelector("[data-rack-id]");rackId=rack?.dataset.rackId||null;scrollEl=e.target.closest?.(".rackMiniCascade")||null;startScroll=scrollEl?.scrollTop||0;scrolling=false;start={x:e.clientX,y:e.clientY,cell,pointerId:e.pointerId};if(rackId)pressTimer=setTimeout(()=>{if(zoomGesture()){clear();return}dragging=true;scrolling=false;state.dragRack=rackId;cell.setPointerCapture?.(e.pointerId);cell.classList.add("rack-dragging");},300);});
        grid.addEventListener("pointermove",e=>{if(!start||e.pointerId!==start.pointerId)return;if(zoomGesture()){clear();return}const dx=e.clientX-start.x,dy=e.clientY-start.y,dist=Math.hypot(dx,dy);if(!dragging&&scrollEl&&!scrolling&&Math.abs(dy)>7&&Math.abs(dy)>Math.abs(dx)*1.15){scrolling=true;if(pressTimer)clearTimeout(pressTimer);pressTimer=null;try{start.cell.setPointerCapture?.(e.pointerId)}catch(_){}}if(scrolling&&!dragging&&scrollEl){e.preventDefault();scrollEl.scrollTop=startScroll-dy;return}if(!dragging&&dist>12){if(pressTimer)clearTimeout(pressTimer);pressTimer=null;}if(dragging){const target=nearestCell(grid,e.clientX,e.clientY);onPreview?.(rackId,target,Engine().neighborhood(rackId));}});
        grid.addEventListener("pointerup",e=>{if(!start||e.pointerId!==start.pointerId)return;if(zoomGesture()){document.querySelectorAll(".rack-dragging").forEach(x=>x.classList.remove("rack-dragging"));state.dragRack=null;clear();return;}if(scrolling&&!dragging){document.querySelectorAll(".rack-dragging").forEach(x=>x.classList.remove("rack-dragging"));state.dragRack=null;clear();return;}const target=nearestCell(grid,e.clientX,e.clientY);if(dragging&&rackId&&target){try{Engine().moveRack(rackId,target.row,target.col);}catch(err){global.dispatchEvent(new CustomEvent("multisynth-rack-ui-error",{detail:err.message}));}}else if(Math.hypot(e.clientX-start.x,e.clientY-start.y)<=12){if(rackId){state.selectedRack=rackId;onOpenRack?.(rackId);}else if(target)onEmptyCell?.(target.row,target.col);}document.querySelectorAll(".rack-dragging").forEach(x=>x.classList.remove("rack-dragging"));state.dragRack=null;clear();});
