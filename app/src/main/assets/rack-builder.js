...
(function(global){
    ...
    function bindGrid(grid,{onOpenRack,onEmptyCell,onPreview}={}){
        ...
        grid.addEventListener("pointerdown",e=>{const cell=e.target.closest("[data-rack-row][data-rack-col]");if(!cell)return;const rack=cell.querySelector("[data-rack-id]");rackId=rack?.dataset.rackId||null;start={x:e.clientX,y:e.clientY,cell};if(rackId)pressTimer=setTimeout(()=>{dragging=true;state.dragRack=rackId;cell.setPointerCapture?.(e.pointerId);cell.classList.add("rack-dragging");},300);});
        grid.addEventListener("pointermove",e=>{if(!start)return;if(!dragging&&Math.hypot(e.clientX-start.x,e.clientY-start.y)>12){if(pressTimer)clearTimeout(pressTimer);pressTimer=null;}if(dragging){const target=nearestCell(grid,e.clientX,e.clientY);onPreview?.(rackId,target,Engine().neighborhood(rackId));}});
        grid.addEventListener("pointerup",e=>{const target=nearestCell(grid,e.clientX,e.clientY);if(dragging&&rackId&&target){try{Engine().moveRack(rackId,target.row,target.col);}catch(err){global.dispatchEvent(new CustomEvent("multisynth-rack-ui-error",{detail:err.message}));}}else if(start&&Math.hypot(e.clientX-start.x,e.clientY-start.y)<=12){if(rackId){state.selectedRack=rackId;onOpenRack?.(rackId);}else if(target)onEmptyCell?.(target.row,target.col);}document.querySelectorAll(".rack-dragging").forEach(x=>x.classList.remove("rack-dragging"));state.dragRack=null;clear();});
