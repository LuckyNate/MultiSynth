...
const STORAGE_KEY = "Stinger.PersistentState.v1";
const TAU = Math.PI * 2;
...
const parameterDefinitions = [
    ...
    { name: "acceleration", label: "ACCELERATION", minimum: 0, maximum: 100, step: 1, circular: false },
...
function loadState() {
    try {
        ...
        saved.channels?.slice(0, 3).forEach((channel, index) => {
            ...
            channelState[index].acceleration = Math.round(clamp(Number(channel.acceleration) || 0, 0, 100));
...
function noteOn(note, velocity = TOUCH_VELOCITY, _overrideState = null, voiceKey = null) {
    ...
    voices.set(key, new StingerVoice(note, velocity, key));
...
function buildChannels() {
    ...
    channelState.forEach((state, channelIndex) => {
        ...
        title.textContent = `STING ${channelIndex + 1}`;
...
function formatParameter(name, value) {
    ...
    if (name === "acceleration" || name === "amount") return `${value}%`;
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootStinger, { once: true });
} else bootStinger();
