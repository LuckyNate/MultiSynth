...
try{
let P=(parent!==window&&parent.MultiSynth)||window.MultiSynth||{},I=P.ModuleIds,E=P.RackEngine,A=P.RackAudioGraph,B=P.ModuleBuilderDefinitions;
if(instance&&E)try{for(const r0 of E.graph?.().racks||[]){const m=r0.modules?.find?.(x=>x.id===instance);if(m){type=m.type;rackId=r0.id;state=m.state||{};break}}}catch(_){}
if(!I||!B){
  ...
  P=window.MultiSynth||{};I=P.ModuleIds;E=P.RackEngine;A=P.RackAudioGraph;B=P.ModuleBuilderDefinitions;
}
...
function patch(p){state={...state,...p};if(rackId&&instance&&E)try{E.setModuleState(rackId,instance,p)}catch(e){console.error(e)}}
