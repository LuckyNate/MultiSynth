...
function refreshStepControls(){for(const slot of stepControls){try{const oldWrap=slot.rendered?.closest?.(".ruiControl");if(!oldWrap?.parentNode)continue;const holder=document.createElement("div"),rendered=U.renderControl(holder,slot.control,{state,defaults:def.defaults||{},patch,audio:A}),newWrap=rendered?.closest?.(".ruiControl")||holder.firstElementChild;if(!newWrap)continue;oldWrap.replaceWith(newWrap);slot.rendered=rendered}catch(e){console.error("Step control refresh failed",e)}}}
function patch(p){state={...state,...p};if(instance)try{if(rackId&&E)E.setModuleState(rackId,instance,p);else if(C)state=C.update(instance,p)}catch(e){console.error(e)}if(Object.prototype.hasOwnProperty.call(p,"selected"))refreshStepControls()}
