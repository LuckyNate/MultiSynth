...
function subscribeTick(fn){if(typeof fn!=="function")return()=>{};const wasEmpty=listeners.size===0;listeners.add(fn);if(wasEmpty){lastFrame=null;setDelivery(true)}let closed=false;return()=>{if(closed)return;closed=true;listeners.delete(fn);if(listeners.size===0){lastFrame=null;setDelivery(false)}}}
...
MS.PatchTransport=Object.freeze({setBpm,resetPhase,subscribeTick,ingestTimebase,snapshot,get bpm(){return bpm},get phase(){return phase},get substep(){return substep},get active(){return listeners.size>0}});
})(window);
