...
(function(global){
  ...
  function resolveType(MS){const I=MS.ModuleIds;if(!I)return null;const key=script?.dataset?.moduleKey;if(key&&I[key])return I[key];const q=new URLSearchParams(global.location.search),direct=q.get("type");if(direct&&I.has(direct))return I.canonicalId(direct);const instance=q.get("instance");if(instance&&MS.RackEngine?.graph)try{for(const rack of MS.RackEngine.graph().racks||[]){const mod=(rack.modules||[]).find(m=>m.id===instance);if(mod)return I.canonicalId(mod.type)}}catch(_){}return null;}
