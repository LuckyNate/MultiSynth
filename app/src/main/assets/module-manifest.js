...
(function(global){
  ...
  function cascadeFor(capabilities,overrides={}){
    const audioIn=has(capabilities,"audioInput"),audioOut=has(capabilities,"audioOutput"),generator=has(capabilities,"generator"),terminal=has(capabilities,"terminalOutput"),clockSource=has(capabilities,"clockSource"),clockFollower=has(capabilities,"clockFollower"),dv=has(capabilities,"dvInput"),cvIn=has(capabilities,"cvInput"),cvOut=has(capabilities,"cvOutput"),notes=has(capabilities,"noteInput");
    const base={
      audioRole:terminal?"terminal":generator?"generator":audioIn&&audioOut?"processor":audioOut?"generator":"none",
      carrierBehavior:!audioIn?"none":generator?"add":terminal?"passthrough":"transform",
      stereoBehavior:audioIn||audioOut?"stereoPreserve":"none",
      timingRole:dv?"dividerConsumer":clockSource&&clockFollower?"sourceFollower":clockSource?"source":clockFollower?"follower":"none",
      cvBehavior:cvIn&&cvOut?"passthrough":cvIn?"moduleSpecific":cvOut?"source":"none",
      voiceMode:Object.freeze({mode:notes?"moduleSpecific":"none",maxVoices:null,steal:"none"}),
      bypassBehavior:audioIn&&audioOut?"passthrough":"silence",
      latencySamples:0
    };
    return Object.freeze({...base,...overrides,voiceMode:Object.freeze({...base.voiceMode,...(overrides.voiceMode||{})})});
  }
  const row=(id,category,color,capabilities=[],resources=[],cascade={})=>{const ident=I.identityFor(id);if(!ident)throw new Error("Missing module identity: "+id);const caps=Object.freeze(capabilities.slice());return Object.freeze({id:ident.id,displayName:ident.displayName,editorUrl:ident.editorUrl,moduleScript:ident.moduleScript,themeKey:ident.id,category,color,capabilities:caps,resources:Object.freeze(resources.slice()),cascade:cascadeFor(caps,cascade)});};
  const M=Object.freeze({
    [I.LIVE_WIRE]:row(I.LIVE_WIRE,"input","#72d7ff",["audioOutput","generator"],["nativeAudio"]),
    [I.BEAT_RED]:row(I.BEAT_RED,"rhythm","#d64b4b",["audioInput","audioOutput","clockFollower","dvInput","cvInput"],[],{carrierBehavior:"add"}),
    ...
    [I.WHITMAN]:row(I.WHITMAN,"sampler","#6b3f24",["audioInput","audioOutput","noteInput","clockFollower","dvInput","cvInput","pcm","mic"],["pcm","mic","storage"],{carrierBehavior:"add"}),
    [I.TIME_BANDITS]:row(I.TIME_BANDITS,"clock","#c89b52",["audioInput","audioOutput","clockSource","clockFollower","dvInput","cvInput","cvOutput"],[],{carrierBehavior:"add"}),
