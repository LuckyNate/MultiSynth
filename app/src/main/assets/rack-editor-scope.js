...
function attach(){
 ...
 /* Detach any standalone renderer from the visible scope. Its old animation loop may continue, but only on the now-hidden old canvas. */
 const fresh=canvas.cloneNode(true);canvas.replaceWith(fresh);canvas=fresh;
 doc.documentElement.dataset.rackScope="post-process";try{doc.defaultView.__MULTISYNTH_RACK_SCOPE_ACTIVE__=true;}catch(_){}
 const ctx=rt.context,an=ctx.createAnalyser();an.fftSize=2048;an.smoothingTimeConstant=0;try{rt.output.connect(an);}catch(_){return;}
 ...
 draw();cleanup=()=>{dead=true;if(raf)global.cancelAnimationFrame(raf);try{rt.output.disconnect(an);}catch(_){}try{an.disconnect();}catch(_){}try{if(doc?.defaultView)doc.defaultView.__MULTISYNTH_RACK_SCOPE_ACTIVE__=false;}catch(_){}};
}
