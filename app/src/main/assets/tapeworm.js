...
const STATE_KEY="tapeworm-basic-v6";
...
let tape=null,tapeLength=0,head=0;
...
function cellIndex(pos){return ((Math.floor(pos)%tapeLength)+tapeLength)%tapeLength}

function buildTapeEngine(){
  tapeLength=Math.max(1,Math.round(ctx.sampleRate*BASE_LOOP_SECONDS));
  tape=new Float32Array(tapeLength);head=0;recordScope.fill(0);recordScopeWrite=0;clearNativeQueue();
  ...
  processor.onaudioprocess=e=>{
    ...
    for(let i=0;i<output.length;i++){
      const idx=cellIndex(head);

      // PLAY HEAD: read the cell exactly once before changing it.
      const old=clamp(tape[idx]);
      output[i]=old;

      // ERASE HEAD: literal attenuation of that tape cell.
      // At 100% falloff keep=0, therefore old content is exactly zero here.
      const erased=clamp(old*keep);
      tape[idx]=erased;

      // RECORD HEAD: overwrite this cell with the attenuated old content + fresh mic.
      // This is assignment, not interpolation/addition into neighboring cells.
...
async function startInput(){
  ...
  if(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia){try{
    ...
    micSource=ctx.createMediaStreamSource(micStream);micSource.connect(processor);statusEl.textContent="TAPE RUNNING // DIRECT MIC";return;
...
async function startTape(){
  ...
  try{const A=window.AudioContext||window.webkitAudioContext;if(!A)throw new Error("Web Audio unavailable");ctx=new A({latencyHint:"interactive"});await ctx.resume();running=true;buildTapeEngine();await startInput();runButton.textContent="STOP TAPE";runButton.classList.add("active");drawScope()}
...
async function stopTape(preserveError=false){
  ...
  clearNativeQueue();nativeMode=false;if(micStream)micStream.getTracks().forEach(t=>t.stop());micStream=null;
  try{micSource&&micSource.disconnect()}catch(_){}micSource=null;
  try{processor&&(processor.onaudioprocess=null,processor.disconnect())}catch(_){}processor=null;
  try{silentKeepAlive&&silentKeepAlive.stop&&silentKeepAlive.stop()}catch(_){}try{silentKeepAlive&&silentKeepAlive.disconnect&&silentKeepAlive.disconnect()}catch(_){}silentKeepAlive=null;
  if(ctx&&ctx.state!=="closed")try{await ctx.close()}catch(_){}ctx=null;tape=null;tapeLength=0;head=0;recordScope.fill(0);recordScopeWrite=0;
...
function drawScope(){
  const w=scope.clientWidth,h=scope.clientHeight;scopeCtx.fillStyle="#fff4ef";scopeCtx.fillRect(0,0,w,h);
  if(running){scopeCtx.strokeStyle="#ffd84a";scopeCtx.lineWidth=3;scopeCtx.beginPath();const n=recordScope.length;for(let i=0;i<n;i++){const idx=(recordScopeWrite+i)%n,s=recordScope[idx],x=i/(n-1)*w,y=h*.5-s*(h*.45/CEILING);i?scopeCtx.lineTo(x,y):scopeCtx.moveTo(x,y)}scopeCtx.stroke()}
  scopeCtx.strokeStyle="#c83f78";scopeCtx.lineWidth=1;scopeCtx.beginPath();scopeCtx.moveTo(0,h/2);scopeCtx.lineTo(w,h/2);scopeCtx.stroke();if(running)drawHandle=requestAnimationFrame(drawScope);
}
