...
/* TAPEWORM — continuous 4-second PCM ring, partitioned into variable grains.
   RECORD is permanently real-time 1x.
   PLAY is independent and relative to recorded PCM.
   SEGMENT never reallocates audio; it only changes logical grain boundaries.
*/
const LOOP_SECONDS=4;
const MIN_SEGMENT_MS=50,MAX_SEGMENT_MS=125;
const MIN_PLAY_SPEED=.4,MAX_PLAY_SPEED=8;
const STATE_KEY="tapeworm-granular-v6",PROCESS_FRAMES=1024,CEILING=.92;
...
let ring=null,loopSamples=0,recordPos=0,playPos=0;
...
function segmentSamples(){return ctx?Math.max(1,Math.round(ctx.sampleRate*segmentMs()/1000)):Math.max(1,Math.round(48000*segmentMs()/1000))}
function segmentCount(){return Math.ceil((ctx?loopSamples:Math.round(48000*LOOP_SECONDS))/segmentSamples())}
function loopSecondsPlayback(){return LOOP_SECONDS/speed()}
function updateReadouts(){const s=speed(),ms=segmentMs(),f=falloffAmount(),t=threshold(),sec=loopSecondsPlayback(),count=segmentCount();speedValue.textContent=`${s.toFixed(3)}×`;segmentValue.textContent=`${Math.round(ms)} ms`;falloffValue.textContent=`${Math.round(f*100)}%`;thresholdValue.textContent=`${Math.round(t*100)}%`;speedReadout.textContent=`PLAY ${s.toFixed(3)}× // ${sec>=10?sec.toFixed(1):sec.toFixed(2)} s LOOP`;segmentReadout.textContent=`${count} SEGMENTS × ${Math.round(ms)} ms`}
...
function allocateRing(){loopSamples=Math.max(1,Math.round(ctx.sampleRate*LOOP_SECONDS));ring=new Float32Array(loopSamples);recordPos=0;playPos=0;recordScope.fill(0);recordScopeWrite=0}

function readPlaySample(){const p=playPos,i0=Math.floor(p)%loopSamples,i1=(i0+1)%loopSamples,f=p-Math.floor(p);return clamp(ring[i0]+(ring[i1]-ring[i0])*f)}
function advancePlay(){playPos+=speed();while(playPos>=loopSamples)playPos-=loopSamples}

function recordSample(fresh){
  const keep=1-falloffAmount();
  // This is the replacement rule for the logical segment currently under the record head.
  // Each cell of that segment is attenuated before the new real-time mic sample is added.
  const stored=clamp(ring[recordPos]*keep+fresh);
  ring[recordPos]=stored;
  recordScope[recordScopeWrite++%recordScope.length]=stored;
  recordPos++;if(recordPos>=loopSamples)recordPos=0;
}

function buildEngine(){allocateRing();clearNativeQueue();gateEnv=gateGain=gateHold=0;processor=ctx.createScriptProcessor(PROCESS_FRAMES,1,1);processor.onaudioprocess=e=>{if(!running)return;const input=e.inputBuffer.numberOfChannels?e.inputBuffer.getChannelData(0):null,out=e.outputBuffer.getChannelData(0);for(let i=0;i<out.length;i++){
    // PLAY first from the continuous ring.
    out[i]=readPlaySample();advancePlay();
    // RECORD is always one real sample per frame, independent of playback speed or grain size.
    const raw=clamp(nativeMode?pullNativeSample():(input?input[i]:0));recordSample(gateMic(raw));
  }};processor.connect(ctx.destination)}

async function startInput(){nativeMode=false;if(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia){try{micStream=await navigator.mediaDevices.getUserMedia({audio:{channelCount:1,echoCancellation:true,noiseSuppression:false,autoGainControl:false}});micSource=ctx.createMediaStreamSource(micStream);micSource.connect(processor);statusEl.textContent="WORM RUNNING // CONTINUOUS 4s RING";return}catch(e){console.warn("direct mic failed",e)}}if(window.MultiSynthNativeMic&&window.AndroidMidi&&typeof AndroidMidi.startMic==="function"){nativeMode=true;nativeUnsubscribe=MultiSynthNativeMic.subscribe(p=>pushNative(p));if(!MultiSynthNativeMic.start())throw new Error("Microphone unavailable");const z=ctx.createConstantSource();z.offset.value=0;z.connect(processor);z.start();silentKeepAlive=z;statusEl.textContent="WORM RUNNING // CONTINUOUS 4s RING // NATIVE MIC";return}throw new Error("Microphone unavailable")}
async function startWorm(){if(running){await stopWorm();return}try{const A=window.AudioContext||window.webkitAudioContext;if(!A)throw new Error("Web Audio unavailable");ctx=new A({latencyHint:"interactive"});await ctx.resume();running=true;buildEngine();await startInput();runButton.textContent="STOP WORM";runButton.classList.add("active");updateReadouts();drawScope()}catch(e){console.error(e);statusEl.textContent="INPUT ERROR";await stopWorm(true)}}
async function stopWorm(err=false){running=false;cancelAnimationFrame(drawHandle);if(nativeUnsubscribe){try{nativeUnsubscribe()}catch(_){}nativeUnsubscribe=null}if(window.MultiSynthNativeMic)try{MultiSynthNativeMic.stop()}catch(_){}clearNativeQueue();nativeMode=false;if(micStream)micStream.getTracks().forEach(t=>t.stop());micStream=null;try{micSource&&micSource.disconnect()}catch(_){}micSource=null;try{processor&&(processor.onaudioprocess=null,processor.disconnect())}catch(_){}processor=null;try{silentKeepAlive&&silentKeepAlive.stop()}catch(_){}try{silentKeepAlive&&silentKeepAlive.disconnect()}catch(_){}silentKeepAlive=null;if(ctx&&ctx.state!=="closed")try{await ctx.close()}catch(_){}ctx=null;ring=null;loopSamples=0;runButton.textContent="START WORM";runButton.classList.remove("active");if(!err)statusEl.textContent="STOPPED";clearScope();updateReadouts()}
