...
(()=>{
  ...
  const SLOT_COUNT=16,STEP_COUNT=32,MIDI_BASE_NOTE=36,clamp=(v,a,b)=>Math.max(a,Math.min(b,Number(v)||0)),copy=v=>JSON.parse(JSON.stringify(v));
  ...
  function selectSlot(index){send({selectedSample:index,recordSlot:index});}
  function noteOn(index,velocity=127){Contract.noteOn(instance,MIDI_BASE_NOTE+index,velocity);}
  function noteOff(index){Contract.noteOff(instance,MIDI_BASE_NOTE+index);}
  ...
  const runSwitch=switchControl("running","RUN PATTERN","running"),previewSwitch=switchControl("previewPlaying","PLAY SELECTED","previewPlaying");
  ...
  bindGlobalKnob("swing","SWING","swing",0,100,1,"%");bindGlobalKnob("length","LENGTH","steps",1,32,1,"");

  const slotNodes=[];
  for(let index=0;index<SLOT_COUNT;index++){
    const node=mount(slots,{id:`sample-${index}`,control:"pad",label:String(index+1).padStart(2,"0"),meta:{midiNote:MIDI_BASE_NOTE+index}},{variant:"square"});
    node.classList.add("whitman-slot");node.dataset.slot=String(index);
    node.addEventListener("multisynth-control-pad-tap",()=>{selectSlot(index);noteOn(index,127);noteOff(index);});
    node.addEventListener("multisynth-control-pad-press",()=>{selectSlot(index);noteOn(index,127);});
    node.addEventListener("multisynth-control-pad-release",()=>noteOff(index));
    slotNodes.push(node);
  }

  const paramNodes=new Map(),paramSpecs={pitch:{label:"PITCH",control:"knob",min:-24,max:24,step:1,unit:" st"},level:{label:"LEVEL",control:"knob",min:0,max:1,step:.01,unit:""},stereo:{label:"STEREO",control:"fader",min:-1,max:1,step:.01,unit:""}};
  function ensureParamNode(key){
    if(paramNodes.has(key))return paramNodes.get(key);
    const spec=paramSpecs[key],node=mount(params,{id:key,control:spec.control,label:spec.label,value:{default:key==="level"?1:0,min:spec.min,max:spec.max,step:spec.step},meta:{unit:spec.unit,bipolar:key==="stereo"}},{variant:spec.control==="fader"?"vertical":"cap",valueReadout:true});paramNodes.set(key,node);return node;
  }
  function bindSelectedParams(){
    const index=selected();
    for(const [key,spec] of Object.entries(paramSpecs)){
      const node=ensureParamNode(key),binding={key:`whitman.sample.${index}.${key}`,get value(){return Number(slotAt(index)[key]??(key==="level"?1:0))},set value(v){updateSample(index,{[key]:v})},get locked(){return !!slotAt(index).locks?.[key]},set locked(v){updateSample(index,{locks:{[key]:!!v}})}};
      node.dataset.bindingKey=binding.key;
      if(spec.control==="fader")Renderer.bindFader(node,binding);else{Renderer.bindKnob(node,binding);node.setControlLocked?.(binding.locked,{silent:true});}
      if(spec.control==="fader")node.commitFaderValue?.(binding.value,{silent:true});else node.setModuleValue?.(binding.value);
    }
  }
  function paintSelectedParams(){const index=selected();for(const [key,node] of paramNodes){const value=Number(slotAt(index)[key]??(key==="level"?1:0));if(paramSpecs[key].control==="fader")node.commitFaderValue?.(value,{silent:true});else{node.setModuleValue?.(value);node.setControlLocked?.(!!slotAt(index).locks?.[key],{silent:true});}}}
  ...
  for(let index=0;index<STEP_COUNT;index++){
    ...
    node.addEventListener("multisynth-control-button-tap",()=>{const sample=selected(),sequence=Array.from({length:STEP_COUNT},(_,step)=>slotAt(sample).sequence?.[step]?127:0);sequence[index]=sequence[index]?0:127;updateSample(sample,{sequence});});stepNodes.push(node);
  }
  ...
  async function choosePCM(id,index=selected()){if(!Library?.get)return;const full=await Library.get(id);if(!full)return;updateSample(index,{name:full.name,pcmKey:full.id,start:0,end:Number(full.duration)||0});}
  ...
  const onStateSync=event=>{
    ...
    if(selectionChanged)refreshSelection({redrawLibrary:true,rebind:true});else{if(selectedSlotChanged){paintSelectedParams();paintSteps();}if(previous.samples!==next.samples)paintSlots();if(keyChanged)drawLibrary().catch(console.error);}
  };
  window.addEventListener("multisynth-state-sync",onStateSync);

  let cleaned=false;function cleanup(){if(cleaned)return;cleaned=true;libraryToken++;parent.removeEventListener("multisynth-pcm-library",onPcmLibrary);parent.removeEventListener("multisynth-grain-library",onGrainLibrary);window.removeEventListener("multisynth-state-sync",onStateSync);}
