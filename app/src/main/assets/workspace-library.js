"use strict";
(function(){
const MS=window.MultiSynth||{},R=MS.RackLibrary,F=MS.RackFaceplate,PROJECT="multisynth.rack.project.v1",items=document.getElementById("items");
...
function createRack(){const p=loadProject(),id=uid("rack");p.racks=p.racks||[];let col=900000;const used=new Set(p.racks.map(r=>`${r.row}:${r.col}`));while(used.has(`900000:${col}`))col++;p.racks.push({id,row:900000,col,enabled:true,gain:1,modules:[]});p.connections=p.connections||[];p.meta=Object.assign({},p.meta,{selectedRack:id,workspaceType:"rack",routing:"explicit-nodes"});saveProject(p);R?.registerRack?.(id);openRack(id)}
function rackRow(r,info){const b=document.createElement("button");b.type="button";b.className="rackLibraryRow";const displayName=info?.name||String(r.id),face=F?.render?.(r,{name:displayName,selector:true});if(face)b.appendChild(face);b.onclick=()=>openRack(r.id);return b}
function render(){items.innerHTML="";const p=loadProject(),named=R?.list?.()||[];for(const r of p.racks||[]){if(String(r.id).startsWith("nodehost-"))continue;const info=named.find(x=>String(x.id)===String(r.id));items.appendChild(rackRow(r,info))}const n=document.createElement("button");n.type="button";n.className="rackLibraryRow new";n.innerHTML='<div><strong>+ NEW EMPTY RACK</strong><small>CREATE A NEW RACK NODE</small></div>';n.onclick=createRack;items.appendChild(n)}
render();
})();