...
(function(global){
  ...
  function directValueReadout(readout,value){if(!readout)return;const target=typeof readout==="string"?document.getElementById(readout):readout?.root||readout;if(!target?.__ms14seg)return;readoutPending.set(target,value);if(!readoutFrame)readoutFrame=requestAnimationFrame(flushReadouts)}
  const FourteenSegmentReadout=Object.freeze({mount:mountFourteenSegmentReadout,valueReadout:directValueReadout,maskFor:readoutMaskFor});
  MS.FourteenSegmentReadout=FourteenSegmentReadout;
  ...
  function mountReadout(host,spec={}){const D=FourteenSegmentReadout;const d=spec?.control?define(spec):define({control:CONTROL.READOUT,id:spec.id,meta:{rows:spec.rows,columns:spec.columns,text:spec.text,lit:spec.lit,name:spec.name,moduleRoot:spec.moduleRoot}});const meta=d.meta||{},moduleRoot=meta.moduleRoot||spec.moduleRoot||null,name=meta.name??spec.name??d.id;const api=D.mount(host,{rows:meta.rows??1,columns:meta.columns??1,text:meta.text??"",lit:!!meta.lit});if(moduleRoot&&name!=null){const scope=readoutScope(moduleRoot),key=String(name);if(scope.has(key))throw new Error("Duplicate readout name in module scope: "+key);scope.set(key,api);api.root.dataset.readoutName=key}return api}
  function valueReadout(moduleRoot,readout,value){if(arguments.length<3){FourteenSegmentReadout.valueReadout(moduleRoot,readout);return}if(!moduleRoot||!readout)return;const target=readoutScopes.get(moduleRoot)?.get(String(readout));if(target)FourteenSegmentReadout.valueReadout(target,value)}
