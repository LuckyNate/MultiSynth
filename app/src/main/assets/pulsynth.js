...
const STORAGE_KEY = "Pulsynth.PersistentState.v1";

const channelState = [
    { octave: 0, detune: 0, duty: 50, amount: 35, phase: 0 },
    { octave: 0, detune: 0, duty: 50, amount: 25, phase: 120 },
    { octave: 0, detune: 0, duty: 50, amount: 20, phase: 240 }
];
...
const parameterDefinitions = [
    ...
    { name: "duty", label: "DUTY", minimum: 1, maximum: 99, step: 1, circular: false },
...
function loadState() {
    try {
        ...
        saved.channels?.slice(0, 3).forEach((channel, index) => {
            ...
            channelState[index].duty = Math.round(clamp(Number(channel.duty) || 50, 1, 99));
...
function makePulseCurve(dutyPercent, phaseDegrees) {
    ...
    const duty = clamp(dutyPercent / 100, .01, .99);
...
class PulseVoice {
    ...
    createStage(index) {
        ...
        input.connect(mixer);
        modulator.connect(pulseShaper);
        pulseShaper.connect(amountGain);
        amountGain.connect(mixer);
        mixer.connect(outputShaper);
        this.modulators.push(modulator);

        return { input, output: outputShaper, modulator, pulseShaper, amountGain };
    }
...
function noteOn(note, velocity = TOUCH_VELOCITY, _overrideState = null, voiceKey = null) {
    ...
    voices.set(key, new PulseVoice(note, velocity, key));
...
function buildChannels() {
    ...
    channelState.forEach((state, channelIndex) => {
        ...
        title.textContent = `PWM ${channelIndex + 1}`;
...
function formatParameter(name, value) {
    ...
    if (name === "duty" || name === "amount") return `${value}%`;
...
function bootPulsynth() {
    ...
    initializeScope();
    warmAudioEngine();
    console.log("Pulsynth ready: three-stage PWM ladder enabled.");
}
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootPulsynth, { once: true });
} else bootPulsynth();
