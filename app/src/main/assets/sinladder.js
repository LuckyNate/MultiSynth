...
const STORAGE_KEY = "SinLadder.PersistentState.v1";
const TAU = Math.PI * 2;

const channelState = [
    { octave: 0, detune: 0, harmonic: 1, amount: 35, phase: 0 },
    { octave: 0, detune: 0, harmonic: 2, amount: 25, phase: 120 },
    { octave: 0, detune: 0, harmonic: 3, amount: 20, phase: 240 }
];

const channelDescriptions = [
    "FUNDAMENTAL SINE STAGE",
    "SECOND HARMONIC STAGE",
    "THIRD HARMONIC STAGE"
];
...
const parameterDefinitions = [
    ...
    { name: "harmonic", label: "HARMONIC", minimum: 1, maximum: 8, step: 1, circular: false },
...
function loadState() {
    try {
        ...
        saved.channels?.slice(0, 3).forEach((channel, index) => {
            ...
            channelState[index].harmonic = Math.round(clamp(Number(channel.harmonic) || index + 1, 1, 8));
...
function noteOn(note, velocity = TOUCH_VELOCITY, _overrideState = null, voiceKey = null) {
    ...
    voices.set(key, new SinVoice(note, velocity, key));
...
function buildChannels() {
    ...
    channelState.forEach((state, channelIndex) => {
        ...
        title.textContent = `SIN ${channelIndex + 1}`;
...
function bootSinLadder() {
    ...
    initializeScope();
    warmAudioEngine();
    console.log("SinLadder ready: three-stage sine ladder enabled.");
}
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootSinLadder, { once: true });
} else bootSinLadder();
