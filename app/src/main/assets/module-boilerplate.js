...
(function(global){
  ...
  const PORT=Object.freeze({CLOCK_IN:"clockIn",CLOCK_OUT:"clockOut",CARRIER_IN:"carrierIn",CARRIER_OUT:"carrierOut",CARRIER_BYPASS:"carrierBypass"});
  const ROOT_PORTS=Object.freeze([]);
  const CHILDREN=Object.freeze({
    [ROLE.ROOT]:Object.freeze({role:ROLE.ROOT,ports:ROOT_PORTS}),
    [ROLE.PROCESSOR]:Object.freeze({role:ROLE.PROCESSOR,ports:Object.freeze([PORT.CARRIER_IN,PORT.CARRIER_OUT,PORT.CARRIER_BYPASS])}),
    [ROLE.GENERATOR]:Object.freeze({role:ROLE.GENERATOR,ports:Object.freeze([PORT.CARRIER_OUT])}),
    [ROLE.HYBRID]:Object.freeze({role:ROLE.HYBRID,ports:Object.freeze([PORT.CARRIER_IN,PORT.CARRIER_OUT,PORT.CARRIER_BYPASS])}),
    [ROLE.UTILITY]:Object.freeze({role:ROLE.UTILITY,ports:ROOT_PORTS}),
    [ROLE.TERMINAL]:Object.freeze({role:ROLE.TERMINAL,ports:Object.freeze([PORT.CARRIER_IN])})
  });
  ...
  function describe(role){const s=requireRole(role);return Object.freeze({role:s.role,ports:s.ports.slice(),root:Object.freeze({identity:true,state:true,lifecycle:lifecycle.slice(),theme:true,decal:true})})}
  ...
  function decal(identity={},override={}){const text=String(override.text||identity.displayName||identity.id||"").trim();return Object.freeze({type:override.type||"text",text,asset:override.asset||null,alt:override.alt||text,placement:override.placement||"top",repeat:Object.freeze(["module-face","node-face","selector"])});}
  function theme(identity={},color="#777",override={}){const d=decal(identity,override.decal||{});return Object.freeze({key:override.key||identity.themeKey||identity.id||"module",color:override.color||color,decal:d,decalTint:"var(--module-decal-tint,#fff)"});}
  ...
  MS.ModuleBoilerplate=Object.freeze({ROLE,PORT,ROOT_PORTS,CHILDREN,lifecycle,require:requireRole,infer,hasPort,describe,define,root,processor,generator,hybrid,utility,terminal,decal,theme,carrier});
})(window);
