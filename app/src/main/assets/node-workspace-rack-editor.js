...
world.addEventListener("click",e=>{const card=e.target.closest?.('.nodeCard[data-node-kind="rack"]');if(!card)return;if(card.dataset.nodeMoved==="1"){card.dataset.nodeMoved="0";return}e.preventDefault();e.stopImmediatePropagation();open(card.dataset.nodeId)},true);
