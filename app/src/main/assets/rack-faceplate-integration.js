...
function nameFor(id){try{return R?.list?.()?.find(x=>String(x.id)===String(id))?.name||`Rack ${String(id).slice(-6).toUpperCase()}`}catch(_){return`Rack ${String(id).slice(-6).toUpperCase()}`}}
