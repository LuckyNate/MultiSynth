...
let ctx=null,collector=null,master=null,limiter=null,masterAnalyser=null,started=false,queued=false;let links=[],mixerBuses=[];
...
function clearLinks(){for(const[a,b]of links)try{a.disconnect(b)}catch(_){}links=[];for(const b of mixerBuses)try{b.disconnect()}catch(_){}mixerBuses=[]}
function link(a,b){if(!a||!b)return;try{a.connect(b);links.push([a,b])}catch(e){console.error("Node audio link",e)}}
function mixerBus(index,state,soloActive){const b=ctx.createGain(),muted=!!state?.mute||(soloActive&&!state?.solo),level=Math.max(0,Math.min(1,Number(state?.level??1)));b.gain.value=muted?0:level;b.connect(collector);mixerBuses[index]=b;return b}
function rebuildNow(){queued=false;ensureContext();clearLinks();const g=E().graph(),by=new Map(g.modules.map(m=>[m.id,m])),incoming=new Set();for(const e of g.connections||[]){if(e.type!=="audio")continue;const a=E().parseNode(e.from),b=E().parseNode(e.to);if(a?.signal!=="carrier"||b?.signal!=="carrier")continue;if(b.kind==="module")incoming.add(b.id)}for(const m of g.modules){const rt=runtime(m);if(rt?.rack)rt.rack.hasUpstream=incoming.has(m.id);try{C().update(m.id,{})}catch(_){}}
const mx=g.mixer?.channels||{},soloActive=Object.values(mx).some(c=>c?.solo);for(const e of g.connections||[]){if(e.type!=="audio")continue;const a=E().parseNode(e.from),b=E().parseNode(e.to),ma=by.get(a?.id);if(!ma||a.signal!=="carrier"||!b||b.signal!=="carrier")continue;const ra=runtime(ma);if(b.kind==="mixer"){link(ra?.output,mixerBus(b.index,mx[b.index]||{},soloActive));continue}const mb=by.get(b.id);if(!mb)continue;link(ra?.output,runtime(mb)?.input)}}
