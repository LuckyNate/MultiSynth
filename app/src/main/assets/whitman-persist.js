...
(function(){
const DB="multisynth-whitman",STORE="pcm",STATE="whitman-working-state-v2";
...
function snapshot(){return {version:2,bpm:+bpmInput.value||120,selectedSample,selectedClipMode,samples:samples.map(s=>({name:s.name,start:s.start,end:s.end,leftLevel:s.leftLevel,rightLevel:s.rightLevel,lagMs:s.lagMs,spatialLock:s.spatialLock,hasPCM:!!s.pcm})),steps:pattern.map(x=>[...x]),clips:clipPattern.map(x=>[...x.entries()])}}
...
async function restore(){let state=null;try{state=JSON.parse(localStorage.getItem(STATE)||"null")}catch(_){state=null}if(!state){restoring=false;return}try{
 for(let i=0;i<samples.length;i++){const p=await getPCM(i);if(p&&p.pcm){installPCM(i,new Float32Array(p.pcm),p.sampleRate,p.name);const m=state.samples&&state.samples[i];if(m){Object.assign(samples[i],{name:m.name||samples[i].name,start:Math.max(0,Math.min(m.start||0,samples[i].buffer.duration)),end:Math.max(0,Math.min(m.end||samples[i].buffer.duration,samples[i].buffer.duration)),leftLevel:m.leftLevel??1,rightLevel:m.rightLevel??1,lagMs:m.lagMs||0,spatialLock:!!m.spatialLock})}}}
