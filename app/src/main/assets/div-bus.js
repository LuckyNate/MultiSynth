...
function locate(g,moduleId){for(const r of g.racks||[]){const active=(r.modules||[]).filter(m=>m.enabled!==false),i=active.findIndex(m=>m.id===moduleId);if(i>=0)return{rack:r,index:i,active}}return null}
...
function send(sourceModuleId,packet={}){const g=E().graph(),p=locate(g,sourceModuleId);if(!p)return false;const out=Object.assign({kind:"div",source:sourceModuleId,time:MS.RackAudioGraph?.context?.currentTime||0,serial:Date.now()+":"+Math.random()},packet||{});walk(g,p.rack.id,p.index+1,out,new Set());for(const child of children(g,p.rack.id))walk(g,child,0,clone(out),new Set());return true}
