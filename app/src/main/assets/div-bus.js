...
function acceptsDiv(m){const meta=M()?.get(m.type);return !!(meta&&K()?.has(meta,K()?.DIV_INPUT))}
...
function walk(g,rackId,start,packet,seen){const key=rackId+":"+start+":"+(packet?.serial||"");if(seen.has(key))return;seen.add(key);const rack=(g.racks||[]).find(r=>r.id===rackId);if(!rack)return;const active=(rack.modules||[]).filter(m=>m.enabled!==false);let out=clone(packet);for(let i=Math.max(0,start|0);i<active.length&&out!=null;i++){if(!acceptsDiv(active[i]))continue;const rt=runtime(active[i].id,rack);if(!rt)continue;const fn=rt.user?.onDiv;if(typeof fn==="function"){try{const v=fn(out);if(v===false||v===null)out=null;else if(v!==undefined)out=v}catch(e){console.error("DIV",e)}}}if(out==null)return;for(const child of children(g,rackId))walk(g,child,0,clone(out),seen)}
