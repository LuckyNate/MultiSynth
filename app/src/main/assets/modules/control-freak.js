...
function midiMessage(){return true}
C.define({type:I.CONTROL_FREAK,version:"control-freak-placeholder-2",description:"PERFORMANCE KEYBOARD · OSCILLOSCOPE · REAL MIDI LIAISON",defaults:defaults(),resources:["midi"],create,setState,destroy,midiMessage,serialize:({state})=>({...state,pitch:0}),restore:({saved})=>Object.assign(defaults(),saved||{},{pitch:0})});
C.defineSurface(I.CONTROL_FREAK,{version:5,package:{id:I.CONTROL_FREAK,version:5,behavior:{role:"performance-controller-liaison",inputs:["hardware-midi","midi"],outputs:["midi"],performanceSurface:"control-keyboard.js",stateOwnership:"module"}},faceplate:{livery:"controller-blue",primary:"#071527",secondary:"#75b7ff",tertiary:"#e1f1ff"},defaults:defaults(),controls:[{id:"scope",control:"oscilloscope",label:"",node:"monitor.scope"}],sources:[{id:"source.touch",type:"performanceInput"},{id:"source.midi",type:"midiInput"}],actions:[{id:"action.note",type:"midiOutput"}],nodes:{connections:[]}});
})(window);
...
if(keyboardHost){const audio={resume:()=>A?.resume?.(),noteOn:(n,v)=>A?.noteOnFrom?.(instance,n,v),noteOff:n=>A?.noteOffFrom?.(instance,n),retuneNote:()=>0,panic:()=>0};PK?.mount?.(keyboardHost,{audio})}
})(window);
