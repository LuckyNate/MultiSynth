...
let state={channel:1,octave:0,velocity:127,pitch:0,mod:0,ribbon:.5,xyX:.5,xyY:.5,knobs:Array(8).fill(.5),faders:Array(8).fill(.75),pads:Array(16).fill(0),mappings:{},...(module.state||{})};
...
function openMapper(key){mapKey=key;mapList.innerHTML="";const current=state.mappings?.[key],title=document.createElement("strong");title.className="cfMapTitle";title.textContent=`MAP ${key.toUpperCase()}${current?` · CURRENT ${current.moduleName} / ${labelPath(current.path)}`:""}`;mapList.appendChild(title);if(current)sharedButton(mapList,"clear-map","CLEAR MAPPING",()=>{const m={...(state.mappings||{})};delete m[key];patch({mappings:m});openMapper(key)});for(const t of targets()){const heading=document.createElement("div");heading.className="cfMapModule";heading.textContent=t.module.displayName||t.module.type;mapList.appendChild(heading);for(const leaf of t.leaves){const meta=inferRange(leaf.path,leaf.value),active=current?.moduleId===t.module.id&&JSON.stringify(current.path)===JSON.stringify(leaf.path);sharedButton(mapList,`map-${t.module.id}-${leaf.path.join("-")}`,`${labelPath(leaf.path)} · ${leaf.value}`,()=>{const m={...(state.mappings||{}),[key]:{moduleId:t.module.id,moduleName:t.module.displayName||t.module.type,path:leaf.path,min:meta.min,max:meta.max,bool:!!meta.bool}};patch({mappings:m});show(`${key.toUpperCase()} MAPPED`);openMapper(key)},active)}}}

const perf=bank("PERFORMANCE","ms-layout-pads"),pads=[];
for(let i=0;i<16;i++){const n=mount(perf,{id:`pad-${i}`,control:"pad",label:String(i+1).padStart(2,"0")},{variant:"square"}),note=36+i;pads.push(n);const down=e=>{if(e.button!=null&&e.button!==0)return;A?.resume?.();A?.noteOn?.(note,state.velocity||127);const arr=[...(state.pads||Array(16).fill(0))];arr[i]=1;patch({pads:arr})},up=()=>{A?.noteOff?.(note);const arr=[...(state.pads||Array(16).fill(0))];arr[i]=0;patch({pads:arr})};n.addEventListener("pointerdown",down);n.addEventListener("pointerup",up);n.addEventListener("pointercancel",up)}
