...
(function(global){
  ...
  const emptySlot=i=>({name:`SAMPLE ${String(i+1).padStart(2,"0")}`,pcmKey:null,start:0,end:0,pitch:0,level:1,stereo:0,sequence:emptySequence()});
  const defaults=()=>({
    swing:0,steps:32,running:false,recording:false,recordSlot:0,selectedSample:0,previewPlaying:false,
  ...
  function normalizedState(saved={}){
    ...
    delete next.bpm;delete next.locks;delete next.stepsData;
    return next;
  }
  ...
  function playbackSlot(source,velocity){
    const amount=clamp(Math.abs(Number(source.stereo)||0),0,1),right=(Number(source.stereo)||0)>0;
    const quiet=1-.6*amount,lag=.025*amount;
    return {...source,level:Math.max(0,Number(source.level??1))*clamp(Number(velocity)||127,1,127)/127,leftLevel:right?quiet:1,rightLevel:right?1:quiet,lagMs:right?lag:-lag};
  }
  function play(runtime,index,time=runtime.ctx.currentTime,velocity=127){
    index=clamp(index,0,SLOT_COUNT-1);
    return runtime.player.play(index,playbackSlot(runtime.state.samples[index]||{},velocity),time);
  }

  function receiveNote(runtime,status,note,velocity=0,time=runtime.ctx.currentTime){
    const n=Math.round(Number(note));if(!Number.isFinite(n))return false;
    const index=n-MIDI_BASE_NOTE;if(index<0||index>=SLOT_COUNT)return false;
    const kind=status&0xf0;
    if(kind===MIDI.NOTE_ON&&Number(velocity)>0)return play(runtime,index,time,velocity);
    if(kind===MIDI.NOTE_OFF||(kind===MIDI.NOTE_ON&&Number(velocity)===0))return true;
    return false;
  }

  function emitInternalNote(runtime,status,note,velocity,time){
    const packet={status,data:[status&255,note&127,velocity&127],time,source:I.WHITMAN_SAMPLER};
    runtime.emit?.("midi",packet);
    return receiveNote(runtime,status,note,velocity,time);
  }

  function releasePatternNotes(runtime,time){
    for(const note of runtime.patternNotes)emitInternalNote(runtime,MIDI.NOTE_OFF,note,0,time);
    runtime.patternNotes.clear();
  }
  ...
  function onTransportMidi(runtime,event={}){
    ...
    if(status===MIDI.STOP){runtime.transportRunning=false;releasePatternNotes(runtime,event.time??runtime.ctx.currentTime);return;}
    if(status!==MIDI.CLOCK||!runtime.transportRunning||!runtime.state.running)return;
  ...
  function create(api){
    ...
    hydrate(runtime);
    if(api.state.recording)runtime.capture.start();if(api.state.previewPlaying)startPreview(runtime);
    return runtime;
  }
  ...
  function setState({runtime,state,patch}){
    ...
    if(Object.prototype.hasOwnProperty.call(patch,"running")&&!state.running)releasePatternNotes(u,u.ctx.currentTime);
  }
  ...
  function destroy({runtime}){const u=runtime.user;if(!u)return;stopPreview(u);releasePatternNotes(u,u.ctx.currentTime);u.unsubscribeMidi?.();for(let i=0;i<SLOT_COUNT;i++)u.loadSerial[i]++;u.capture?.destroy();u.player.stopAll();try{u.player.buffers?.clear?.();}catch(_){}for(const node of [u.input,u.through,u.samplerOut,u.mix,u.output])try{node.disconnect();}catch(_){}}

  C.define({type:I.WHITMAN_SAMPLER,version:9,description:"WHITMAN SAMPLER · 16 PCM SLOTS · MIDI NOTES 36–51 · 32-STEP MIDI PATTERN SETTER · SHARED MIDI REALTIME",defaults:defaults(),resources:["pcm","storage","midi"],create,setState,noteOn,noteOff,destroy,serialize:({state})=>normalizedState(state),restore:({saved})=>normalizedState(saved)});
  C.defineSurface(I.WHITMAN_SAMPLER,{version:9,package:{id:I.WHITMAN_SAMPLER,version:9,behavior:{role:"16-slot-midi-sampler-pattern-setter",audioMode:"additive-pass-through",clockMode:"shared-midi-realtime",midiNotes:"36-51-map-to-sample-slots",stateOwnership:"module"}},faceplate:{livery:"whitman-sampler",primary:"#3b2118",secondary:"#f1dfbd",tertiary:"#9d6a45"},defaults:defaults(),controls:[
