...
const defaults={};
C.define({
 type:I.LIVE_WIRE,version:"module-builder-14",description:"FREESOUND AUDIO PLAYER · CV/MANUAL SAMPLE TRIGGER · 30 RPM PRECISION SEEK · HOLD OR COPY TO SAMPLE",defaults,
 create(api){
  ...
  let duration=0,transportOffset=0,transportStartedAt=0,transportPlaying=false,transportToken=0,transportTimer=null;
  ...
  const clearTimer=()=>{if(transportTimer)clearTimeout(transportTimer);transportTimer=null};
  const markTransport=(offset,t)=>{clearTimer();transportOffset=Math.max(0,Math.min(duration,Number(offset)||0));transportStartedAt=Number(t)||ctx.currentTime;transportPlaying=true;const token=++transportToken,ms=Math.max(0,((transportStartedAt-ctx.currentTime)+(duration-transportOffset))*1000);transportTimer=setTimeout(()=>{if(token!==transportToken)return;transportPlaying=false;transportOffset=0;transportStartedAt=0;emitTransport({ended:true})},ms+12);emitTransport()};
  ...
  const stop=(reset=true)=>{transportPlaying=false;transportStartedAt=0;transportOffset=reset?0:currentOffset();transportToken++;clearTimer();player.stopAll();emitTransport()};
