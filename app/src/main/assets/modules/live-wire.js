...
C.define({
 type:I.LIVE_WIRE,version:"module-builder-2",description:"YOUTUBE CARRIER · LIVE · HOLD TO SAMPLE",defaults,
 create(api){
  ...
  const off=MS.LiveWireNative?.onChunk(chunk)||null;return{out,rackGain,off};
 },
 setState({runtime,state}){const u=runtime.user;if(!u)return;const t=runtime.context?.currentTime||0;try{u.rackGain.gain.setTargetAtTime(state.live?1:0,t,.008)}catch(_){u.rackGain.gain.value=state.live?1:0}},
