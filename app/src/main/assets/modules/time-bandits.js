...
const model=Defs.require(I.TIME_BANDITS),clamp=(v,a,b)=>Math.max(a,Math.min(b,Number(v)||0)),LOOKAHEAD=.75;
...
function consumeExternalPulse(u,p={},mode="cv"){
 ...
 u.divCount=0;
 if(ratio===1){emitDv(u,time,sourcePeriod,p.source||null,0);return}
 const count=Math.max(1,Math.round(ratio)),step=sourcePeriod/count;for(let i=0;i<count;i++)emitDv(u,time+i*step,step,p.source||null,i)
}
function internalClock(u){if(performance.now()<u.externalUntil)return;const now=u.ctx.currentTime,interval=(60/bpm(u))/speed(u);if(u.nextInternalAt==null)u.nextInternalAt=now+.02;if(u.nextInternalAt<now){const missed=Math.floor((now-u.nextInternalAt)/interval)+1;u.nextInternalAt+=missed*interval}while(u.nextInternalAt<=now+LOOKAHEAD){emitDv(u,u.nextInternalAt,interval,u.id,0);u.nextInternalAt+=interval}}
function schedulerTick(u){internalClock(u)}
function resetTiming(u){u.divCount=0;u.nextInternalAt=null}
function create(api){const c=api.context,input=c.createGain(),voice=c.createGain(),mix=c.createGain(),output=c.createGain();input.connect(mix);voice.connect(mix);mix.connect(output);api.setInput(input);api.setOutput(output);const u={id:api.instanceId,ctx:c,input,voice,mix,output,state:api.state,sampleBuffer:null,internalBuffer:null,onDv:null,onDiv:null,externalUntil:0,dvUntil:0,nextInternalAt:null,timer:null,lastSourcePeriod:null,divCount:0};buildInternalSample(u);u.onDv=p=>{consumeExternalPulse(u,p,"dv");return null};u.onDiv=u.onDv;u.timer=setInterval(()=>schedulerTick(u),25);if(api.state.pcmKey)load(u,api.state.pcmKey);return u}
function setState({runtime,state,patch}){const u=runtime.user;if(!u)return;const old=speed(u);u.state=state;if("bpm" in patch||("division" in patch&&speed(u)!==old))resetTiming(u);if("pitch" in patch||"decay" in patch||"tone" in patch)buildInternalSample(u);if("pcmKey" in patch)load(u,state.pcmKey)}
...
C.define({type:I.TIME_BANDITS,version:"12",description:"PREBUILT PCM CLOCK · GAP-RESISTANT AUDIO-TIMESTAMP SCHEDULING · LITERAL CV/DV DIVISION · DIRECT MULTIPLICATION · DV PRIORITY",defaults:defaults(),resources:["storage"],create,setState,clockStart,clockStop,clockTick,cv,destroy,moduleBuilder:model});
})(window);
