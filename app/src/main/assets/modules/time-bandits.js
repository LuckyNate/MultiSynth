...
function startInternal(u,fireNow=true){u.externalSeen=false;u.dvSeen=false;clearInternalTimer(u);const interval=internalInterval(u);u.internalIntervalSec=interval;u.lastInternalMs=performance.now();if(fireNow)emitDv(u,u.ctx.currentTime,interval,u.id);armInternal(u,interval)}
...
function create(api){const c=api.context,input=c.createGain(),voice=c.createGain(),mix=c.createGain(),output=c.createGain();input.connect(mix);voice.connect(mix);mix.connect(output);api.setInput(input);api.setOutput(output);const u={id:api.instanceId,ctx:c,input,voice,mix,output,state:api.state,sampleBuffer:null,internalBuffer:null,onDv:null,onDiv:null,lastPeriod:null,count:0,dvSeen:false,externalSeen:false,internalTimer:null,lastInternalMs:0,internalIntervalSec:0};buildInternalSample(u);u.onDv=p=>{u.dvSeen=true;u.externalSeen=true;stopInternal(u);pulse(u,p,"dv");return null};u.onDiv=u.onDv;if(api.state.pcmKey)load(u,api.state.pcmKey);startInternal(u,true);return u}
...
function clockStop({runtime}){const u=runtime.user;if(!u)return;stopInternal(u);u.count=0;u.dvSeen=false;u.externalSeen=false}
...
C.define({type:I.TIME_BANDITS,version:"14",description:"PERFORMANCE RETEMPO · PREBUILT PCM · DETERMINISTIC POWER-OF-TWO CV/DV · DV PRIORITY · NO GRAPH REBUILD",defaults:defaults(),resources:["storage"],create,setState,clockStart,clockStop,clockTick,cv,destroy,moduleBuilder:model});
})(window);
