...
const MULTIPLIERS=new Map([[2,2],[4,4],[8,8],[16,16],[32,32]]);
...
function speed(u){const n=Number(u.state.division);return DIVISORS.has(n)||MULTIPLIERS.has(n)||n===1?n:1}
...
function pulse(u,p={},source="clock"){
 const mode=speed(u),time=Number(p.time)||u.ctx.currentTime,packetPeriod=Math.max(.001,Number(p.period)||60/(Number(p.bpm)||bpm(u))),measured=u.lastInputTime!=null&&time>u.lastInputTime?time-u.lastInputTime:0,period=measured>.001?measured:(u.lastPeriod||packetPeriod);
 u.lastInputTime=time;u.lastPeriod=period;
 if(source==="dv"){u.dvSeen=true}else if(u.dvSeen)return;
 const divisor=DIVISORS.get(mode)||0;
 if(divisor){u.count++;if(u.count>=divisor){u.count=0;fire(u,time,period*divisor,p.source||null)}return}
 u.count=0;
 if(mode===1){fire(u,time,period,p.source||null);return}
 const multiplier=MULTIPLIERS.get(mode)||1,step=period/multiplier;
 for(let i=0;i<multiplier;i++){const target=time+i*step;hit(u,target);sendDvAt(u,target,step,p.source||null,i)}
}
...
C.define({type:I.TIME_BANDITS,version:"18",description:"EXPLICIT POWER-OF-TWO DIVIDE/MULTIPLY MATH · TOP-TIMER CLOCK SOURCE · UPSTREAM FOLLOWER · AUDIO-CLOCK PCM · DV PRIORITY",defaults:defaults(),resources:["storage"],create,setState,clockStart,clockStop,clockTick,cv,destroy,moduleBuilder:model});
})(window);
