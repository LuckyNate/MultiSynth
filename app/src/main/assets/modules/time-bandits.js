...
function pulse(u,p={},source="cv"){
 ...
 const n=Math.round(ratio),step=period/n;
 for(let i=0;i<n;i++){const target=time+i*step;hit(u,target);sendDvAt(u,target,step,p.source||null,i)}
}
function clearInternalTimer(u){if(u.internalTimer){clearTimeout(u.internalTimer);u.internalTimer=null}}
function armInternal(u,delaySec){clearInternalTimer(u);if(u.dvSeen||u.externalSeen)return;u.internalTimer=setTimeout(()=>{u.internalTimer=null;if(u.dvSeen||u.externalSeen)return;const now=performance.now();u.lastInternalMs=now;const interval=internalInterval(u);u.internalIntervalSec=interval;fire(u,u.ctx.currentTime,interval,u.id);armInternal(u,interval)},Math.max(1,delaySec*1000))}
function startInternal(u,fireNow=true){u.externalSeen=false;u.dvSeen=false;clearInternalTimer(u);clearDvTimers(u);u.lastInputTime=null;const interval=internalInterval(u);u.internalIntervalSec=interval;u.lastInternalMs=performance.now();if(fireNow)fire(u,u.ctx.currentTime,interval,u.id);armInternal(u,interval)}
...
function create(api){const c=api.context,input=c.createGain(),voice=c.createGain(),mix=c.createGain(),output=c.createGain();input.connect(mix);voice.connect(mix);mix.connect(output);api.setInput(input);api.setOutput(output);const u={id:api.instanceId,ctx:c,input,voice,mix,output,state:api.state,sampleBuffer:null,internalBuffer:null,onDv:null,onDiv:null,lastPeriod:null,lastInputTime:null,count:0,dvSeen:false,externalSeen:false,internalTimer:null,lastInternalMs:0,internalIntervalSec:0,dvTimers:[]};buildInternalSample(u);u.onDv=p=>{u.dvSeen=true;u.externalSeen=true;stopInternal(u);pulse(u,p,"dv");return null};u.onDiv=u.onDv;if(api.state.pcmKey)load(u,api.state.pcmKey);startInternal(u,true);return u}
...
function clockStart({runtime}){const u=runtime.user;if(!u)return;u.count=0;startInternal(u,false)}
function clockStop({runtime}){const u=runtime.user;if(!u)return;stopInternal(u);clearDvTimers(u);u.count=0;u.dvSeen=false;u.externalSeen=false;u.lastInputTime=null}
...
function destroy({runtime}){const u=runtime.user;if(!u)return;stopInternal(u);clearDvTimers(u);for(const n of [u.input,u.voice,u.mix,u.output])try{n.disconnect()}catch(_){}}
C.define({type:I.TIME_BANDITS,version:"16",description:"AUDIO-CLOCK-LOCKED MULTIPLIER · REAL-TIME DV EMISSION · PERFORMANCE RETEMPO · PREBUILT PCM · DV PRIORITY",defaults:defaults(),resources:["storage"],create,setState,clockStart,clockStop,clockTick,cv,destroy,moduleBuilder:model});
})(window);
