...
(function(global){
  ...
  const MIDI_BASE=36,STEPS=32,VOICES=16,clamp=(v,a,b)=>Math.max(a,Math.min(b,Number(v)||0)),copy=v=>JSON.parse(JSON.stringify(v));
  const BASE=[
  ...
  const defaults=()=>({bpm:120,swing:0,running:false,fill:false,selectedVoice:0,selectedStep:0,lockMode:false,tick:0,lanes:Array.from({length:VOICES},(_,i)=>lane(i))});

  function normalize(state){
    state.bpm=clamp(state.bpm??120,20,300);state.swing=clamp(state.swing??0,0,100);state.running=!!state.running;state.fill=!!state.fill;state.lockMode=!!state.lockMode;state.selectedVoice=clamp(Math.round(state.selectedVoice||0),0,VOICES-1);state.selectedStep=clamp(Math.round(state.selectedStep||0),0,STEPS-1);state.tick=Math.max(0,Math.floor(Number(state.tick)||0));
  ...
  function fireStep(u,packet){if(!u.state.running)return;const tick=u.state.tick++,stepDur=60/u.state.bpm/4,odd=tick%2===1,swingDelay=odd?stepDur*.5*(u.state.swing/100):0;
    for(let i=0;i<VOICES;i++){const lane=u.state.lanes[i];if(!laneEnabled(u.state,i))continue;const s=lane.steps[tick%lane.length];if(!s?.on)continue;if(s.fill&&!u.state.fill)continue;if(Math.random()*100>=s.probability)continue;const micro=stepDur*(s.micro/100),base=Math.max(u.ctx.currentTime+.001,(Number(packet.time)||u.ctx.currentTime)+swingDelay+micro),count=s.ratchet,gap=stepDur/count,vel=clamp(s.velocity,1,127);for(let r=0;r<count;r++)hitSynth(u,i,base+r*gap,s,vel)}
  }
  function create(api){const c=api.context,input=c.createGain(),drums=c.createGain(),mix=c.createGain(),output=c.createGain();normalize(api.state);input.connect(mix);drums.connect(mix);mix.connect(output);api.setInput(input);api.setOutput(output);const u={ctx:c,input,drums,mix,output,state:api.state,unsub:null};T.setBpm(api.state.bpm);u.unsub=T.subscribeTick(p=>fireStep(u,p));return u}
  function setState({runtime,state,patch}){const u=runtime.user;if(!u)return;normalize(state);u.state=state;if("bpm" in patch)T.setBpm(state.bpm);if("running" in patch){if(state.running){state.tick=0;T.start()}else T.stop()}}
  ...
  function panic({runtime}){return true}
  function destroy({runtime}){const u=runtime.user;if(!u)return;u.unsub?.();for(const n of[u.input,u.drums,u.mix,u.output])try{n.disconnect()}catch(_){}}

  const surfaceDefaults=defaults();
  C.define({type:I.TIME_BANDITS,version:"time-bandits-3",description:"16-VOICE MIDI DRUM SYNTH · 32 STEPS · PER-STEP PROBABILITY / VELOCITY / MICROTIMING / RATCHET / PARAMETER LOCKS · MIDI NOTES 36–51",defaults:surfaceDefaults,resources:["midi"],create,setState,noteOn,noteOff,panic,destroy,serialize:({state})=>copy(state),restore:({saved})=>normalize({...defaults(),...(saved||{})})});
  C.defineSurface(I.TIME_BANDITS,{version:3,package:{id:I.TIME_BANDITS,version:3,behavior:{role:"midi-drum-machine",steps:32,voices:16,probability:"per-step",stateOwnership:"module"}},faceplate:{livery:"time-bandits",primary:"#24160f",secondary:"#d9a84f",tertiary:"#f4dfad"},defaults:surfaceDefaults,controls:[]});
