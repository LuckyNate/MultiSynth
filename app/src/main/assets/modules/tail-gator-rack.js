...
function apply(u,s){
  const on=s.normalize!==false,now=u.ctx.currentTime;
  u.normalizer.threshold.setTargetAtTime(on?-20:0,now,.01);
  u.normalizer.knee.setTargetAtTime(on?12:0,now,.01);
  u.normalizer.ratio.setTargetAtTime(on?3:1,now,.01);
  const pct=clamp(s.tailLift??160,100,250);
  u.lift.gain.setTargetAtTime(pct/100,now,.015);
}
...
C.define({
  ...
  restore:({saved})=>({armed:false,sinkId:"",normalize:true,tailLift:160,...(saved||{})})
});
})(window);
