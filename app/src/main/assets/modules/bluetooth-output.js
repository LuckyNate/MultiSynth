...
const defaults={};
function create(api){const c=api.context,input=c.createGain(),output=c.createGain();input.connect(output);api.setInput(input);api.setOutput(output);const u={instanceId:api.instanceId,input,output};queueMicrotask(()=>MS.OutputRouting?.routeBluetooth?.(api.instanceId,output));return u}
function destroy({runtime}){const u=runtime.user;if(!u)return;MS.OutputRouting?.remove?.(runtime.instanceId);for(const n of [u.input,u.output])try{n.disconnect()}catch(_){}}
