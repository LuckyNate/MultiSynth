...
C.define({type:I.ALCHEMY_MIXER,version:"module-1",description:"DYNAMIC OUTPUT MIXER",defaults,create,setState({runtime,state}){runtime.user?.sync(state)},destroy({runtime}){for(const g of runtime.user?.inputs?.values?.()||[])try{g.disconnect()}catch(_){}for(const a of runtime.user?.meters?.values?.()||[])try{a.disconnect()}catch(_){}try{runtime.user?.output?.disconnect()}catch(_){}},serialize:({state})=>({channels:{...(state.channels||{})}}),restore:({saved})=>({channels:{...(saved?.channels||{})}})});
...
function render(){root.innerHTML="";meters.length=0;const count=channelCount();status.textContent=`${count-1} CONNECTED · ${count} INPUTS AVAILABLE`;for(let i=0;i<count;i++){
 ...
 const level=mount(row.querySelector(".levelHost"),{id:`level-${i}`,control:C.FADER,label:"LEVEL",value:{default:c.level,min:0,max:1,step:.01}},{variant:"vertical",valueReadout:true}),meter=mount(row.querySelector(".meterHost"),{id:`meter-${i}`,control:C.METER,label:"LEVEL",value:{default:0,min:0,max:1}},{variant:"vertical"}),mute=mount(row.querySelector(".muteHost"),{id:`mute-${i}`,control:C.BUTTON,label:"MUTE"},{variant:"rect"}),solo=mount(row.querySelector(".soloHost"),{id:`solo-${i}`,control:C.BUTTON,label:"SOLO"},{variant:"rect"});
