...
function rebuild(){if(rebuilding)return;rebuilding=true;try{ensureContext();const g=E().graph(),byId=new Map(g.racks.map(r=>[r.id,r])),valid=new Set(byId.keys());cleanup(valid);disconnect(collector);collector.connect(master);for(const r of g.racks){const x=io(r);[...x.parentIn,x.inputMix,x.output,...x.childOut].forEach(disconnect);x.parentIn.forEach(n=>n.connect(x.inputMix));x.output.gain.value=Number.isFinite(r.gain)?r.gain:1;const active=r.modules.filter(m=>m.enabled!==false);let previous=x.inputMix;for(const m of active){const rt=runtime(m.id,r);disconnect(rt.input);disconnect(rt.output);if(rt.input){previous.connect(rt.input);previous=rt.output||rt.input;}else if(rt.output){previous=rt.output;}}previous.connect(x.output);x.childOut.forEach(out=>x.output.connect(out));}for(const edge of g.edges){const p=byId.get(edge.from),c=byId.get(edge.to);if(!p||!c)continue;const cs=relationSlot(p,c),ps=parentSlot(p,c);if(cs===null||ps===null)continue;rackIO.get(p.id)?.childOut[cs].connect(rackIO.get(c.id)?.parentIn[ps]);}const leaves=g.racks.filter(r=>E().neighborhood(r.id).children.length===0);for(const leaf of leaves)rackIO.get(leaf.id)?.output.connect(collector);global.dispatchEvent(new CustomEvent("multisynth-audio-graph",{detail:{type:"rebuilt",racks:g.racks.length,edges:g.edges.length,leaves:leaves.map(r=>r.id)}}));}finally{rebuilding=false;}}
