...
function registerRack(id,name){const d=load();id=String(id);let r=d.racks.find(x=>String(x.id)===id);if(!r){r={id,name:String(name||`Rack ${d.racks.length+1}`),semanticId:""};d.racks.push(r)}else if(name)r.name=String(name);if(typeof r.semanticId!=="string")r.semanticId="";save(d);return clone(r)}
...
function cleanSemanticId(value){return String(value??"").trim().replace(/[\r\n\t]+/g," ").slice(0,80)}
function assignSemanticId(id,value){const d=load(),r=d.racks.find(x=>String(x.id)===String(id));if(!r)return false;r.semanticId=cleanSemanticId(value);save(d);return clone(r)}
...
MS.RackLibrary=Object.freeze({load,list,get,registerRack,rename,assignSemanticId,remove});
})(window);
