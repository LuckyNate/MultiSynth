(function () {
    ...
    function sendPacket(packet={}){const status=Number(packet.status)&255,command=status&0xf0;if(command<0x80||command>0xe0)return false;const data=Array.isArray(packet.data)?packet.data:[],d1=Number(packet.data1??packet.note??packet.control??packet.program??data[1]??0)&127,d2=Number(packet.data2??packet.velocity??packet.value??data[2]??0)&127,length=(command===0xc0||command===0xd0)?2:3;return sendNative(status,d1,d2,length)}
    ...
    window.MultiSynthNativeMidi={receive:process,sendPacket,sendClockPulse,sendStart,sendContinue,sendStop,get usbClockActive(){return MS().PatchTransport?.external===true},devicesChanged:function(){},permissionResult:function(){},status:function(text,connected){const b=document.getElementById("nativeMidiInput");if(!b)return;b.textContent=connected?"USB-C / MIDI ON":"USB-C / MIDI";b.title=text;b.classList.toggle("active",!!connected)},panic:panicAll};
