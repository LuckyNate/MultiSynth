...
auto.onclick=()=>{segments=analyze();if(!segments.length){status.textContent="AUTO FIND: NO SAMPLE BOUNDARIES DETECTED";return}window.MultiSynthTheChopper?.applyAutoSegments?.(segments);status.textContent=`AUTO FOUND ${segments.length} SAMPLE${segments.length===1?"":"S"} · HOLD SEGMENTS TO SELECT · NONE SELECTED SAVES ALL`;};
const oldClear=clear?.onclick;if(clear)clear.onclick=e=>{segments=null;window.MultiSynthTheChopper?.clearSelection?.();oldClear?.call(clear,e)};
})();