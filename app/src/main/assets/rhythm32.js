...
let ctx,master,analyser,playing=false,currentStep=0,nextStepTime=0,timer=null,selected=0,syncMode="internal",openHatGain=null;
...
else if(v.kind==="hato"){let g=ctx.createGain();openHatGain=g;let proxy={...v};for(let k=0;k<5;k++){let o=ctx.createOscillator(),f=ctx.createBiquadFilter();o.type="square";o.frequency.value=Math.min(14000,v.pitch*(1.2+k*.71));f.type="highpass";f.frequency.value=1800+v.tone*65;o.connect(f).connect(g);o.start(t);o.stop(t+v.decay/1000+.1)}env(g,t,(v.level/100)*.22,v.decay);g.connect(master)}
...
function renderEditor(){let v=VOICES[selected];$("voiceName").textContent=v.name;$("voiceType").textContent=v.type;["pitch","decay","bend","tone","character"].forEach(id=>{$(id).value=v[id];$(id).nextElementSibling.textContent=v[id]+(id==="pitch"?" Hz":id==="decay"?" ms":id==="bend"?" ct":"")});$("voiceLevel").value=v.level;$("voiceLevel").nextElementSibling.textContent=v.level}
["pitch","decay","bend","tone","character"].forEach(id=>$(id).addEventListener("input",()=>{VOICES[selected][id]=+$(id).value;$(id).nextElementSibling.textContent=$(id).value+(id==="pitch"?" Hz":id==="decay"?" ms":id==="bend"?" ct":"");save()}));$("voiceLevel").addEventListener("input",()=>{VOICES[selected].level=+$("voiceLevel").value;$("voiceLevel").nextElementSibling.textContent=$("voiceLevel").value;save()});
function stepDuration(step){let base=60/+$("bpm").value/4,sw=+$("swing").value/100;return base*(step%2?1+sw*.5:1-sw*.5)}
function schedule(){if(!playing||syncMode!=="internal")return;let horizon=ctx.currentTime+.12;while(nextStepTime<horizon){let len=+$("steps").value;let s=currentStep%len;VOICES.forEach((v,i)=>{if(pattern[i][s])hit(i,nextStepTime)});let shown=s;setTimeout(()=>{currentStep=shown;paintPlayhead()},Math.max(0,(nextStepTime-ctx.currentTime)*1000));nextStepTime+=stepDuration(s);currentStep=(s+1)%len}timer=setTimeout(schedule,25)}
function start(){ensureAudio();playing=true;currentStep=0;nextStepTime=ctx.currentTime+.03;$("play").textContent="STOP";$("play").classList.add("active");if(syncMode==="internal")schedule();$("status").textContent=syncMode==="internal"?"Running on internal clock.":"Waiting for external sync pulses."}
function stop(){playing=false;clearTimeout(timer);$("play").textContent="PLAY";$("play").classList.remove("active");document.querySelectorAll(".step.playhead").forEach(x=>x.classList.remove("playhead"))}
function paintPlayhead(){document.querySelectorAll(".step.playhead").forEach(x=>x.classList.remove("playhead"));document.querySelectorAll(`.step[data-s="${currentStep}"]`).forEach(x=>x.classList.add("playhead"))}
$("play").addEventListener("click",()=>playing?stop():start());$("preview").addEventListener("click",()=>hit(selected));$("clear").addEventListener("click",()=>{pattern=VOICES.map(()=>Array(32).fill(0));buildGrid();save()});
["bpm","swing","steps"].forEach(id=>{$(id).nextElementSibling.textContent=$(id).value;$(id).addEventListener("input",()=>{$(id).nextElementSibling.textContent=$(id).value;if(id==="steps")buildGrid();save()})});
$("sync").addEventListener("click",()=>{syncMode=syncMode==="internal"?"external":"internal";$("sync").textContent="SYNC: "+syncMode.toUpperCase();if(playing){stop();start()}save()});
window.MultiSynthRhythm32={
 setTempo(bpm){$("bpm").value=Math.max(40,Math.min(240,bpm));$("bpm").nextElementSibling.textContent=$("bpm").value;save()},
 setSync(mode){syncMode=mode==="external"?"external":"internal";$("sync").textContent="SYNC: "+syncMode.toUpperCase();save()},
 pulse(time){if(syncMode!=="external"||!playing)return;ensureAudio();let len=+$("steps").value,s=currentStep%len,t=Number.isFinite(time)?time:ctx.currentTime;VOICES.forEach((v,i)=>{if(pattern[i][s])hit(i,t)});currentStep=(s+1)%len;paintPlayhead()},
 start(){if(!playing)start()},stop(){if(playing)stop()}
};
const scope=$("scope"),sg=scope.getContext("2d");function sizeScope(){scope.width=scope.clientWidth*devicePixelRatio;scope.height=scope.clientHeight*devicePixelRatio}sizeScope();addEventListener("resize",sizeScope);function drawScope(){if(!analyser){requestAnimationFrame(drawScope);return}let d=new Uint8Array(analyser.fftSize);analyser.getByteTimeDomainData(d);sg.fillStyle="#020303";sg.fillRect(0,0,scope.width,scope.height);sg.strokeStyle="#b8ff3d";sg.lineWidth=2*devicePixelRatio;sg.beginPath();for(let i=0;i<d.length;i++){let x=i/(d.length-1)*scope.width,y=d[i]/255*scope.height;i?sg.lineTo(x,y):sg.moveTo(x,y)}sg.stroke();requestAnimationFrame(drawScope)}
$("sync").textContent="SYNC: "+syncMode.toUpperCase();renderEditor();buildGrid();drawScope();
