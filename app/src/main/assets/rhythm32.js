...
function start(){ensureAudio();playing=true;currentStep=0;playheadStep=0;nextStepTime=ctx.currentTime+.03;$("play").textContent="STOP";$("play").classList.add("active");if(syncMode==="internal")schedule();$("status").textContent=syncMode==="internal"?"Running on internal clock.":"Waiting for external sync pulses."}
...
function paintPlayhead(){document.querySelectorAll(".step.playhead").forEach(x=>x.classList.remove("playhead"));document.querySelectorAll(`.step[data-s="${playheadStep}"]`).forEach(x=>x.classList.add("playhead"))}
...
window.MultiSynthRhythm32={
 ...
 pulse(time){if(syncMode!=="external"||!playing)return;ensureAudio();let len=+$("steps").value,s=currentStep%len,t=Number.isFinite(time)?time:ctx.currentTime;VOICES.forEach((v,i)=>{if(pattern[i][s])hit(i,t)});playheadStep=s;currentStep=(s+1)%len;paintPlayhead()},
