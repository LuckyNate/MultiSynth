...
(function(global){
  ...
  const VARIANTS=freeze({
    knob:freeze(["cap","skirted","pointer","encoder"]),encoder:freeze(["rotary","selector","indexed"]),turntable:freeze(["platter"]),fader:freeze(["vertical","horizontal"]),ribbon:freeze(["horizontal","vertical"]),expression:freeze(["horizontal","vertical"]),pad:freeze(["square","hex"]),button:freeze(["rect","round","arcade"]),switch:freeze(["rocker","slide","toggle","vertical"]),xy:freeze(["pad"]),readout:freeze(["glass","amber","inset","banner"]),screen:freeze(["screen","scroll"]),oscilloscope:freeze(["scope"]),meter:freeze(["bar","needle"]),led:freeze(["round","rect"]),jack:freeze(["socket"]),decal:freeze(["screenprint","sticker","stencil","plate"])
  });
