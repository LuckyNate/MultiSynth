...
(function(global){
  ...
  const TYPES=freeze({
    ...
    [C.WHEEL]:freeze({variant:"vertical",width:42,height:112,touchWidth:56,touchHeight:124,centerDetent:false,springReturn:false,labelGap:8,valueReadout:true}),
  ...
  const VARIANTS=freeze({
    ...
    dial:freeze(["rotary","selector","indexed"]),
    wheel:freeze(["vertical","horizontal"]),
    fader:freeze(["vertical","horizontal"]),
    pad:freeze(["square","round","strip"]),
  ...
  function cssVars(control,overrides={}){const s=resolve(control,overrides),out={};for(const [k,v] of Object.entries(s)){
    if(typeof v==="number"){
      const unitless=new Set(["travel","startAngle","endAngle","ticks"]);
      out[`--ms-${k.replace(/[A-Z]/g,m=>"-"+m.toLowerCase())}`]=unitless.has(k)?String(v):px(v);
    }else if(typeof v==="boolean") out[`--ms-${k.replace(/[A-Z]/g,m=>"-"+m.toLowerCase())}`]=v?"1":"0";
    else if(v!=null)out[`--ms-${k.replace(/[A-Z]/g,m=>"-"+m.toLowerCase())}`]=String(v);
  }return freeze(out)}
