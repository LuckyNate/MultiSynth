"use strict";
(function(){
const MS=window.MultiSynth||{},R=MS.ControlSurfaceRenderer,C=MS.ControlSurface?.CONTROL,root=document.getElementById("test-controls");if(!R||!C||!root)return;
...
const mount=(host,control,label,extra={})=>R.mount(host,{id:`test-${control}-${host.children.length}`,control,label,...extra});
let row;
...
row=bank("8 · KEYS",8);for(let i=0;i<8;i++)mount(row,C.KEY,`${i+1}`);
