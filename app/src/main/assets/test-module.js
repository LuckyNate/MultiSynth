...
if(readoutRoot){const add=(text,columns,look,word=false)=>{const box=document.createElement("div");box.className="readout-prototype"+(word?" word":"");readoutRoot.appendChild(box);CS.mountReadout(box,{control:C.READOUT,variant:look,meta:{rows:1,columns,text,look}})};add("1",1,"glass");add("02",2,"amber");add("003",3,"inset");add("MultiSynth",10,"banner",true);add("Welcome to MultiSynth, the node based modular synth for Android!",12,"watch",true)}
