"use strict";
(function(){
const MS=window.MultiSynth||{},R=MS.ControlSurfaceRenderer,C=MS.ControlSurface?.CONTROL,SPEC=MS.ControlSurfaceSpec,root=document.getElementById("test-controls"),readoutRoot=document.getElementById("readout-prototypes"),D=MS.FourteenSegmentReadout;if(!R||!C||!SPEC||!root)return;
if(readoutRoot&&D){const add=(text,columns,look,word=false)=>{const box=document.createElement("div");box.className="readout-prototype"+(word?" word":"");readoutRoot.appendChild(box);D.mount(box,{rows:1,columns,text,look})};add("1",1,"glass");add("02",2,"amber");add("003",3,"inset");add("MultiSynth",10,"banner",true)}
