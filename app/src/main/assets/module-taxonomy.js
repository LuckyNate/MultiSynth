...
function setFamily(id,family){const all=load(FAMILY_STORE),key=String(id),v=normalizeFamily(family);if(v)all[key]=v;else delete all[key];save(FAMILY_STORE,all);global.dispatchEvent(new CustomEvent("multisynth-module-taxonomy-changed",{detail:{id:key,family:familyFor(key)}}));return familyFor(key)}
...
function userTagsFor(id){const all=load(TAG_STORE),arr=Array.isArray(all[String(id)])?all[String(id)]:[];return arr.map(normalizeTag).filter(Boolean)}
function tagsFor(id){return[...new Set([...automaticTags(id),...userTagsFor(id)])].sort()}
function setUserTags(id,tags){const all=load(TAG_STORE),arr=[...new Set((Array.isArray(tags)?tags:String(tags||"").split(/[\s,]+/)).map(normalizeTag).filter(Boolean))].sort();if(arr.length)all[String(id)]=arr;else delete all[String(id)];save(TAG_STORE,all);global.dispatchEvent(new CustomEvent("multisynth-module-taxonomy-changed",{detail:{id:String(id),tags:arr}}));return arr}
...
MS.ModuleTaxonomy=Object.freeze({familyFor,defaultFamilyFor,setFamily,tagsFor,userTagsFor,setUserTags,matches,normalizeTag,normalizeFamily,families:()=>Object.freeze([...new Set((M?.all||[]).map(x=>familyFor(x.id)))].sort())});
})(window);
