...
(function(global){
  ...
  function createState(orientation="portrait"){
    ...
    return {orientation,count,start:initialStart(count),velocity:VELOCITY_MAX,active:new Map()};
  }
  ...
  function mount(host,{audio=global.parent?.MultiSynth?.RackAudioGraph||MS.RackAudioGraph,orientation=innerWidth>innerHeight?"landscape":"portrait"}={}){
    ...
    const state=createState(orientation),held=new Map();
    host.classList.add("ms-performance-keyboard");
    host.innerHTML='<div class="ms-pk-bezel"><div class="ms-pk-velocity"><div class="ms-pk-velocity-label">VELOCITY</div><div class="ms-pk-velocity-ribbon" role="slider" aria-label="Velocity" aria-valuemin="8" aria-valuemax="127" aria-valuenow="127"><span class="ms-pk-velocity-fill"></span><span class="ms-pk-velocity-thumb"></span></div><output class="ms-pk-velocity-value">127</output></div><div class="ms-pk-viewport"><div class="ms-pk-bed"></div></div></div>';
    const ribbon=host.querySelector(".ms-pk-velocity-ribbon"),readout=host.querySelector(".ms-pk-velocity-value"),viewport=host.querySelector(".ms-pk-viewport"),bed=host.querySelector(".ms-pk-bed");
    ...
    bed.addEventListener("pointerdown",e=>{const key=e.target.closest?.(".ms-pk-key");if(!key)return;e.preventDefault();try{bed.setPointerCapture(e.pointerId)}catch(_){}on(e.pointerId,key)});
    bed.addEventListener("pointermove",e=>{if(!held.has(e.pointerId))return;e.preventDefault();const key=keyAt(e.clientX,e.clientY);if(key)move(e.pointerId,key)});
    const finish=e=>{off(e.pointerId);try{bed.releasePointerCapture(e.pointerId)}catch(_){}};bed.addEventListener("pointerup",finish);bed.addEventListener("pointercancel",finish);bed.addEventListener("lostpointercapture",e=>off(e.pointerId));
    function setRibbon(e){e.preventDefault();setVelocityFromY(state,e.clientY,ribbon.getBoundingClientRect());drawVelocity()}
    ...
    return Object.freeze({host,state,setVelocity:v=>{setVelocity(state,v);drawVelocity()},getVelocity:()=>state.velocity});
  }

  MS.PerformanceKeyboard=Object.freeze({
    MIDI_MIN,MIDI_MAX,MIDDLE_C,PORTRAIT_VISIBLE,LANDSCAPE_VISIBLE,OCTAVE_CS,VELOCITY_MIN,VELOCITY_MAX,
    visibleCount,middleAnchorIndex,startForTarget,startForOctave,initialStart,noteName,isBlack,velocityFromY,
    createState,setOrientation,setVelocity,setVelocityFromY,jumpOctave,panByNotes,visibleNotes,noteOn,moveTouch,noteOff,mount
  });
})(window);
