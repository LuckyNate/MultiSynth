...
final class LiveWireBridge {
    ...
    @JavascriptInterface public boolean startLiveWire() {
        ...
        if (activity.checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            ...
            LiveWireHub.status("RECORD AUDIO PERMISSION REQUIRED");
            return false;
        }
        if (LiveWireProjectionService.isActive()) return true;
        activity.runOnUiThread(() -> {
            try { activity.startActivity(new Intent(activity, LiveWirePermissionActivity.class)); }
            catch (Exception e) { LiveWireHub.status("CAPTURE PERMISSION WINDOW FAILED"); }
        });
        return true;
    }
    ...
    @JavascriptInterface public void youtubeSearch(String query, int requestId, boolean random, int max) {
        YouTubeSearch.execute(query, requestId, random, max);
    }

    void destroy() { LiveWireHub.detach(webView); }
}
